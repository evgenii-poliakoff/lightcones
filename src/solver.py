...
class bath:
    ...
    def from_chain(cls, chain, n_sites = -1, rel_tol = 10**(-3), ring_size = 4, t_max = 100, dt = 0.01, visualize = False):
        ...
        if visualize:
            ...
            H_ = H.todense()
            e_, U_ = tools.find_eigs_ascending(H_)
        ...
        if visualize:
            bth.couplings_rel = U_rel_full.T.conj() @ psi_lc
            bth.U_rel =  np.copy(U_rel_full.T.conj())
            bth.I_rel = np.copy(pi)
        ...
        if visualize:
            ...
            bth.couplings_min = bth.U_min @ psi_lc
        ...
        if visualize:
            ...
            #def m_in(self, ti) :
            #    for i in range(len(self.itimes_in)):
            #        if self.itimes_in[i] > ti:
            #            return i;
            def m_in(self, ti) :
                for i in self.intervals:
                    if  i[0] <= ti and ti < i[1]:
                        return i[3]
                raise ValueError('Interval not found')

            bth.m_in = types.MethodType(m_in, bth)
        ...
        for i in intervals_in:
            ...
            if n_coupled > max_n_coupled:
                ...
                if visualize:
                    ...
                    #bth.couplings_cdia[n_out : n_in, i[0] :] = U.T.conj() @ bth.couplings_cdia[n_out : n_in, i[0] :]
        ...
        for i in intervals_r:
            for j in range(i[0], i[1]):
                ...
                if visualize:
                    bth.couplings_cdia[: n_rel, j] = u @ bth.couplings_cdia[: n_rel, j]
            ...
            def m_out(self, ti):
                for i in self.intervals:
                    if  i[0] <= ti and ti < i[1]:
                        return i[2]
                raise ValueError('Interval not found')
            bth.m_out = types.MethodType(m_out, bth)
        ...
        if visualize:
            def w_cdia(self, ti):
                ...
                for i in self.intervals:
                    if  i[0] <= ti and ti < i[1]:
                        return self.rotations[ni]
                    ni = ni + 1
                raise ValueError('Interval not found')
            bth.w_cdia = types.MethodType(w_cdia, bth)
...
class problem:
    ...
    def construct_model(self, max_num_quanta):
        ...
        m = tools.boson_chain(num_modes = m_max, max_num_quanta = nquanta, id_s = id_s)
        lo = m.get_local_observables()

        id_e = m.space.eye
     
        a_hat = [tools.kron(id_s, a_) for a_ in m.a]
        a_hat_dag = [a_.T.conj() for a_ in a_hat]
    
        O = [tools.kron(o_[1], id_e) for o_ in self.impurity_observables]
        ...
        zero_op = tools.kron(scipy.sparse.csc_array((dim_s, dim_s), dtype = complex), m.space.emptyH) 
